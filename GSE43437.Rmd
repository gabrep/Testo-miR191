...
#Cluster
```{r}
dist <- dist(t(exp_final))
hc <- hclust(dist)
dend <- as.dendrogram(hc)


dend %>% 
  color_labels(labels = colnames(exp_final[,1:4]), col="darkred") %>%
  color_labels(labels = colnames(exp_final[,5:9]), col="darkgreen") %>%
  plot()
...
colnames(design) <- levels(as.factor(pdata$`treatment:ch1`))
